...
phy_id = phy_fn.split(sep="/")[-1].split(sep=".")[0]
...
os.system("awk '{ for (i=1; i < NF; i++) { print \"%s_\"NR\"\t\"$i  }}' %s.edges.mcl > %s.edges.mcl.out" % (phy_id,out_fn, out_fn))
num_clu = sum(1 for line in open('%s.edges.mcl' % out_fn))
