...
gene_col = "node"
clus_col = "og_cluster"
ort = pd.read_csv("/home/xavi/Documents/auto-orthology/orthofinder_Ano14sp/outiqt.orthology.csv", sep="\t")
...
clus_lis = np.unique(evc.cluster)
for c in clu_lis:
	nod_lis = evc[evc["cluster"] == c]["node"].values
	sps_lis = [n.split("_")[0] for n in nod_lis ]
	a = 0
	for s in sps_lis:
...
for s in ["Anogam"]:
	...
	# define empty dict for ages relative to sps s
	clu_age_dic[s] = dict()
	for c in clu_lis:
		clu_age_dic[s][c] = 0
