...
for soi in [0,0.01,0.05,0.1] :
	...
	evou    = np.empty((len(evev)*1000, 4), dtype="object")
	evou[:] = np.nan
	...
	evou_d.columns = ["in_gene","out_gene","ev_type","branch_support"]

	# save edges
	evou_d.to_csv("%s.sos_%.2f.edges" % (out_fn, soi), sep="\t", index=False)
	logging.info("Species overlap score = %.2f, num edges = %i" % (soi,evou_d.shape[0]))
